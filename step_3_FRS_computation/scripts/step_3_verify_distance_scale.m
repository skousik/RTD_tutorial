%% description
% This script demonstrates how to use the distance scale correctly to get
...
P_fp_FRS = world_to_FRS(P_fp_world,A.state(:,end),x0,y0,D) ;
...
P_Z0_world = FRS_to_world(P_Z0_FRS(:,1:5:end),A.state(:,end),x0,y0,D) ;

%% move the world point to the FRS frame and back
p_world_to_FRS = world_to_FRS(p_world,A.state(:,end),x0,y0,D) ;
p_world_to_FRS_to_world = FRS_to_world(p_world_to_FRS,A.state(:,end),x0,y0,D) ;

%% move the FRS point to the world frame and back
p_FRS_to_world = FRS_to_world(p_FRS,A.state(:,end),x0,y0,D) ;
p_FRS_to_world_to_FRS = world_to_FRS(p_FRS_to_world,A.state(:,end),x0,y0,D) ;
